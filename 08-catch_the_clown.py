...
def main():
    ...
    pygame.display.set_caption('08 - catch the clown')
...
def mouse_click(clown_img,
    ...
    """
    global CLOWN_MOVING, SPEED_MULTIPLIER
    ...
    for event in pygame.event.get(): #Check the events on the event queue
        if event.type == MOUSEBUTTONDOWN:
            ...
            if CLOWN_MOVING:
                ...
                speed_x = random.randint(1, 5) * SPEED_MULTIPLIER
                speed_y = random.randint(1, 5) * SPEED_MULTIPLIER
                CLOWN_MOVING = True
